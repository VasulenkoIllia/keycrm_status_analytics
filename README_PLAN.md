...
- **Налаштування циклу**: UI для вибору старт/фініш груп/статусів (cycle rules) на проект.
- **Налаштування SLA**: окрема форма для двох сіток (звичайна/термінова) по групах, прив’язана до `sla_stage_rules`.
- **Налаштування терміновості**: редактор `urgent_rules` (за sku/offer_id/product_id) і ручний override на картці замовлення (пише в `order_overrides`).
...
День 6 (за потреби): UI налаштувань SLA/циклів/urgent_rules, вкладка Аналітика; доведення overrides.
